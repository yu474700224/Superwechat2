...
public class DemoDBManager {
     ...
     * @param user
     */
    synchronized public void savaUserToHXDB(UserAvatar user) {
        SQLiteDatabase db = dbHelper.getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(UserDao.MY_COLUMN_NAME_ID, user.getMUserName());
        values.put(UserDao.MY_COLUMN_NAME_NICK, user.getMUserNick());
        values.put(UserDao.MY_COLUMN_NAME_AVATAR, user.getMAvatarId());
        values.put(UserDao.MY_COLUMN_AVATAR_TYPE, user.getMAvatarType());
        values.put(UserDao.MY_COLUMN_AVATAR_LAST_UPDATA_TIME, user.getMAvatarLastUpdateTime());
        values.put(UserDao.MY_COLUMN_AVATAR_PATH, user.getMAvatarPath());
        if (db.isOpen()) {
            db.replace(UserDao.MY_TABLE_NAME, null, values);
        }
    }

    synchronized public UserAvatar getUserAvatar(String usernmae) {
        SQLiteDatabase db = dbHelper.getReadableDatabase();
        Cursor cursor = db.rawQuery("select * from " + UserDao.MY_TABLE_NAME, null);
        UserAvatar userAvatar = null;
        if (cursor.moveToNext()) {
            userAvatar = new UserAvatar();
            userAvatar.setMUserName(usernmae);
            userAvatar.setMUserNick(cursor.getString(cursor.getColumnIndex(UserDao.MY_COLUMN_NAME_NICK)));
            userAvatar.setMAvatarId(cursor.getInt(cursor.getColumnIndex(UserDao.MY_COLUMN_NAME_AVATAR)));
            userAvatar.setMAvatarLastUpdateTime(cursor.getString(cursor.getColumnIndex(UserDao.MY_COLUMN_AVATAR_LAST_UPDATA_TIME)));
            userAvatar.setMAvatarPath(cursor.getColumnName(cursor.getColumnIndex(UserDao.MY_COLUMN_AVATAR_PATH)));
